...
export class BoardsService {
    ...
    async getMyAllBoards(logginedUser: User): Promise<Board[]> {
        this.logger.verbose(
            `User ${logginedUser.username} is retrieving their own boards`,
        );
    ...
    async getBoardsByKeyword(author: string): Promise<Board[]> {
        ...
        const foundBoards = await this.boardRepository.findBy({
            author: author,
        });
        if (foundBoards.length === 0) {
            throw new NotFoundException(
                `No boards found for author: ${author}`,
            );
        }

        this.logger.verbose(
            `Retrieving boards by author: ${author} Successfully`,
        );
        return foundBoards;
    }
    ...
    async createBoard(
        ...
        this.logger.verbose(
            `User ${logginedUser.username} is creating a new board with title: ${createArticleRequestDto.title}`,
        );
    ...
    async updateBoardStatusById(
        ...
        this.logger.verbose(
            `ADMIN is attempting to update the status of board with ID ${id} to ${status}`,
        );
        ...
        this.logger.verbose(
            `Board with ID ${id} status updated to ${status} by Admin`,
        );
    }
    ...
    async deleteBoardById(id: number, logginedUser: User): Promise<void> {
        this.logger.verbose(
            `User ${logginedUser.username} is attempting to delete board with ID ${id}`,
        );
        ...
        if (foundBoard.user.id !== logginedUser.id) {
            throw new UnauthorizedException(
                'Do not have permission to delete this board',
            );
        }
        await this.boardRepository.delete(foundBoard);

        this.logger.verbose(
            `Board with ID ${id} deleted by User ${logginedUser.username}`,
        );
    }
}
